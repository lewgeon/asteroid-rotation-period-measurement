"""Data models used by the standalone pointing solver."""
...
class CartesianState:
    ...
    def __post_init__(self) -> None:
        position = np.asarray(self.position_km, dtype=np.float64)
        velocity = np.asarray(self.velocity_km_s, dtype=np.float64)
...
class LineOfSight:
    ...
    def __post_init__(self) -> None:
        direction = np.asarray(self.unit_icrs, dtype=np.float64)
        if direction.shape != (3,):
            raise ValueError("unit_icrs must have shape (3,)")
        object.__setattr__(self, "unit_icrs", direction)
