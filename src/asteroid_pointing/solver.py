...
def _unit(vector: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    if distance == 0:
        raise ValueError("Line-of-sight vector has zero length")
...
class LightTimePointingSolver:
    ...
    def solve(
        ...
        if time_role == "transmit":
            return self._solve_from_transmit(target, observation_time)
        if time_role == "receive":
            return self._solve_from_receive(target, observation_time)
        raise ValueError("time_role must be 'transmit' or 'receive'")
