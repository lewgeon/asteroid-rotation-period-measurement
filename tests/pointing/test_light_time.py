...
from asteroid_radar.pointing.models import CartesianState
from asteroid_radar.pointing.solver import C_KM_S, LightTimePointingSolver
...
class LightTimeTests(unittest.TestCase):
    ...
    def test_moving_target(self):
        ...
        expected = distance / (C_KM_S - speed)
        self.assertAlmostEqual(result.uplink_light_time_s, expected, 6)

    def test_receive_time_recovers_transmit_time(self):
        target = LinearTarget(
            self.epoch, [4e7, 2e6, 1e6], [-10, 4, 1]
        )
